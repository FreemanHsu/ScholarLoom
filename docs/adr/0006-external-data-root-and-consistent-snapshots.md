...
`/Users/jun/ScholarLoomData`, and never falls back to a directory inside the
...
- `originals/` stores immutable, content-addressed source bytes such as ingested
  PDFs. These bytes are retained even when the remote source remains available.
...
root before operators switch the application to it. Encryption, off-device
