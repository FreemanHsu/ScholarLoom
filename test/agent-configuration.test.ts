...
describe("Agent configuration registry", () => {
  it("defines the model and thinking budget used by every ScholarLoom Agent task", () => {
    expect(AGENT_CONFIGURATION_VERSION).toBe("agent-configuration.v9");
    expect(listAgentConfigurations().map(({ taskKind, model, reasoningEffort }) =>
      ...
      { taskKind: "paper-version-diff", model: "gpt-5.6-sol", reasoningEffort: "high" },
      ...
      { taskKind: "knowledge-answer", model: "gpt-5.6-sol", reasoningEffort: "medium" },
    ...
    expect(getAgentConfiguration("knowledge-answer").execution).toMatchObject({
      mode: "agentic-curated-knowledge",
      network: "denied",
